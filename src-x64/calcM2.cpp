...
//'
//' @description Calculates the predation mortality for every species in each length class.
...
//' @param ration A matrix with dimensions \code{nsc} and \code{nfish} representing the amount of food required for fish of a given species and length class to move up into the next length class to grow according von Bertalanffy growth curve in a time step.
...
//' @details \code{sum_m(sum_n(I[j,i]*N[j,i]*suit_M2[[m]][n,j,i]/(sum_k(sum_l(suit_M2[[m]][n,l,k]W[l,k]N[l,k])) + other)))}
...
//' @return A matrix with dimensions \code{nsc} and \code{nfish} representing the the predation mortality for every species in each length class.
...
//' nfish <- nrow(NS_par)
//' nsc <- 32
//' maxsize <- max(NS_par$Linf) * 1.01 ## biggest size is 1% bigger than the largest Linf
...
//' W_a <- NS_par$W_a ## the length-weight conversion.
//' W_b <- NS_par$W_b ## the length-weight conversion.
//' k <- NS_par$k ## the von-Bertalnaffy growth parameter.
//' Lmat <- NS_par$Lmat ## the length at which 50\% of the individuals are mature (cm).
//'
//' ### getting phi_min
//' tmp <- calc_phi(k, Linf, nsc, nfish, u_bound, l_bound,calc_phi_min=FALSE,
//'                 phi_min=0.1) # with fixed phi_min
//' phi <- tmp$phi
//' phi_min <- tmp$phi_min
//'
//' # calculate ration growth
...
//' g_eff <- tmp$g_eff
//'
//' ## calculate size prefernce
//' prefs <- calc_prefs(pred_mu = -2.25, pred_sigma = 0.5, wgt, sc_Linf)
//' ## calculated the prey preference and prey suitability
