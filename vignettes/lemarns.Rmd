...
### Recruitment
In @thorpe2015 and @hall recruitment only occurs in the first time step of a new year. The number of individuals after the recruitment phase of the model $N'_{j,i}$ is thus defined as $N'_{j,i}=N_{j,i,t-1}$ for $j=2,\ldots,n_l$ and
\begin{equation*}
N'_{1,i} = N_{1,i,t-1}+R_{i,t},
\end{equation*}
...
### Mortality
The number of individuals after the mortality phase of the model $N''_{j,i}$ is defined as:
\begin{equation*}
N''_{j,i}=N'_{j,i}\exp(-(M1_{j,i}+M2_{j,i}+F_{j,i})).
\end{equation*}
...
\begin{equation*}
M2_{j,i}=\sum_{m=1}^{n_s}\sum_{n=1}^{n_l}I_{j,i}N_{j,i}\frac{U(W_{n,m},W_{j,i},m,i)}{\sum_{k=1}^{n_s}\sum_{l=1}^{n_l}U(W_{n,m},W_{l,k},m,k)W_{l,k}N_{l,k} + o}
\end{equation*}
where $o$ is other food.

The catch of species $i$ in length class $j$ is
\begin{equation*}
C_{j,i}= N'_{j,i}(1-\exp(-F_{j,i})).
\end{equation*}
...
N_{j,i,t}=
\begin{cases}
    N''_{j,i}(1-\phi_{j,i})      & \quad \text{if } j=1\\
    N''_{j,i}(1-\phi_{j,i}) + N''_{j-1,i}\phi_{j-1,i}      & \quad \text{otherwise}
\end{cases}
\end{equation*}
...
There are several ways to use the `run_LeMans` function. All of the parameters can be added to the function explicitly. To do this, we first need to define the initial population and set the correct number of time steps. Below we run the model for 10 years:
```{r}
...
## Fishing fleets
In this case study we explore the mixed fishery example described in @thorpe2016, which involves four different fishing fleets. Here, we use idealised fleets, i.e. a single fish is caught by only one gear type. The data frame `mixed_fish` contains the information on which fleet catches which species:
```{r}
mixed_fish
```
In this example, the selectivity of each species follows the `logistic` curve and requires the parameters `eta` and `L50`, which are provided as part of `LeMaRns`.
...
```
To run the `LeMansParam` function `rec_fish` and `mixed_fish` need to be combined:
```{r}
...
\begin{equation*}
f_i(F_i,F_{-i})=f_i(F_i,F'_{-i})=f_i(F_i),
\end{equation*}
$\forall{}F'_{-i}$ and we can define
\begin{equation*}
F_{MSY,i}={\text{arg}\max}_{F_i}f_i(F_i).
\end{equation*}
